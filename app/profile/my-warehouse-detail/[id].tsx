...
import {
  useGetWarehouseById,
  usePatchWarehouseStatus,
} from "@/query-hooks/useWarehoue";
import {
  AccessTypes,
...
export default function MyWarehouseDetailScreen() {
  ...
  const { data, isLoading, error } = useGetWarehouseById(id ?? "");
  const warehouse = data?.result;
  const { mutate: patchStatus, isPending: isPatching } =
    usePatchWarehouseStatus();
  ...
  const handleToggleStatus = () => {
    ...
    const title = isActive ? "Yayından Kaldır" : "Yayına Al";
    const message = isActive
      ? "Bu ilanı yayından kaldırmak istediğinize emin misiniz?"
      : "Bu ilanı yayına almak istediğinize emin misiniz?";
    ...
    Alert.alert(title, message, [
      { text: "İptal", style: "cancel" },
      {
        ...
        onPress: () => patchStatus({ id: warehouse.id, status: newStatus }),
      },
    ]);
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          style={styles.headerButton}
          onPress={() => router.push(`/warehouse-edit/${warehouse.id}`)}
          hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
        >
      ...
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom + 12 }]}>
        <TouchableOpacity
          style={[
            ...
            warehouse.status === "ACTIVE"
              ? styles.statusToggleDeactivate
              : styles.statusToggleActivate,
          ]}
          activeOpacity={0.8}
        ...
        >
          {isPatching ? (
            <ActivityIndicator
              size="small"
              color={warehouse.status === "ACTIVE" ? "#FF3B30" : "#43A047"}
            />
          ) : (
            <>
              <Ionicons
