...
import { AiChatQueryKeys, useGetAiMessages } from "@/query-hooks/useAiChat";
...
export default function AiChatDetailScreen() {
  ...
  const handlePickImage = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(translate("common.error"), translate("aiChat.uploadFailed"));
    } finally {
  ...
  const handleSend = useCallback(() => {
    ...
    queryClient.setQueriesData<{
      ...
      limit: number;
      count: number;
    }>({ queryKey: [AiChatQueryKeys.MESSAGES, conversationId] }, (previous) => {
      if (!previous) {
  ...
  return (
    ...
    >
      ...
      {/* Messages */}
      <View
        style={[styles.messagesContainer, { backgroundColor: tokens.bgBase }]}
      >
      ...
      >
        ...
        <ChatComposerInput
          ...
          disabled={(!messageText.trim() && !pendingImages.length) || isSending}
          isSending={isSending}
          onSend={handleSend}
          editable={!isSending}
        />
